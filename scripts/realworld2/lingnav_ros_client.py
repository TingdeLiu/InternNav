...
线程架构:
  ...
  规划线程 (0.3s)  — 调用 LingNavPipeline.step() → 更新 MPC 轨迹 / 旋转目标
  控制线程 (0.1s)  — MPC solve / 角速度控制 → 发布 /cmd_vel

启动:
  python scripts/realworld2/lingnav_ros_client.py \\
...
_ROOT = _HERE.parents[1]                            # InternNav root
_REALWORLD = _ROOT / "scripts" / "realworld"        # wheeltec_controllers 所在目录
...
# NavDP 端侧本地推理客户端（--local_s1 时导入）
_NAVDP_DIR = str(_ROOT / "scripts" / "inference" / "NavDP")
if _NAVDP_DIR not in sys.path:
    sys.path.insert(0, _NAVDP_DIR)
...
# LingNavNode
# ─────────────────────────────────────────────────────────────────────────────
class LingNavNode(Node):
    """LingNav ROS2 主节点"""

    def __init__(self, cfg: argparse.Namespace):
        super().__init__("lingnav_node")
        self.cfg = cfg

        # ── 初始化 S1 客户端（本地模式 or 服务器模式）────────────────────────
        s1_client = None
        if cfg.local_s1:
            from navdp_local_client import NavDPLocalClient  # noqa: E402
        ...
        self.get_logger().info(
            f'LingNav started | instruction="{cfg.instruction}" '
    ...
    def stop(self) -> None:
        ...
        self.get_logger().info("[LingNav] Stopped.")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="LingNav Jetson ROS2 客户端",
...
def main():
    ...
    if cfg.local_s1 and not cfg.s1_checkpoint:
        raise SystemExit("[LingNav] --s1_checkpoint 是必填项（使用 --local_s1 时）")
    ...
    rclpy.init()
    node = LingNavNode(cfg)

    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        print("\n[LingNav] KeyboardInterrupt, shutting down …")
    finally:
        node.stop()
        node.destroy_node()
        rclpy.shutdown()
