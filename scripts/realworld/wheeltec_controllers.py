...
try:
    ...
    print("Warning: CasADi not installed. MPC controller will not be available.")
...
class Mpc_controller:
    def __init__(self, global_planed_traj, N=20, desired_v=0.3, v_max=0.4, w_max=0.4, ref_gap=4):
        ...
        if not CASADI_AVAILABLE:
            raise ImportError("CasADi is required for MPC controller. "
    ...
    def find_reference_traj(self, x0, global_planed_traj):
        ...
        Args:
            x0: 当前状态
            global_planed_traj: 全局规划轨迹

        Returns:
            ref_traj_pts: 参考轨迹点数组
        """
        ref_traj_pts = []

        # 检查轨迹是否有效
        if len(global_planed_traj) < 2:
            # 轨迹太短，用最后一个点填充
            for _ in range(self.ref_traj_len):
                ref_traj_pts.append(global_planed_traj[-1, :] if len(global_planed_traj) > 0 else np.zeros(2))
        ...
        # 计算累积距离，在开头添加 0
        segment_dists = np.linalg.norm(np.diff(global_planed_traj, axis=0), axis=1)
        ...
        for i in range(nearest_idx, len(global_planed_traj)):
