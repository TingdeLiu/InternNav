...
# 与 internvla_n1.py 保持一致的 token 占位 id
...
class InternVLAN1ModelConfigQwen3(Qwen3VLConfig):
    model_type = "internvla_n1_qwen3"
...
class InternVLAN1ModelQwen3(InternVLAN1MetaModel, Qwen3VLModel):
    config_class = InternVLAN1ModelConfigQwen3

    def __init__(self, config: InternVLAN1ModelConfigQwen3):
        super(InternVLAN1ModelQwen3, self).__init__(config)


class InternVLAN1ForCausalLMQwen3(Qwen3VLForConditionalGeneration, InternVLAN1MetaForCausalLM):
    config_class = InternVLAN1ModelConfigQwen3

    def __init__(self, config):
        Qwen3VLForConditionalGeneration.__init__(self, config)
        config.model_type == "internvla_n1_qwen3"

        self.model = InternVLAN1ModelQwen3(config)
    ...
    def forward(
        ...
        # Qwen3-VL 的 get_rope_index 返回 3D position_ids (3, B, seq_len)
    ...
    def generate_latents(self, input_ids, pixel_values, image_grid_thw):
        ...
        # Qwen3-VL get_rope_index 兼容仅传 image_grid_thw（video_grid_thw=None, second_per_grid_ts=None）
